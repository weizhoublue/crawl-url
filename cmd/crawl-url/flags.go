...
func printUsage() {
	...
	fmt.Fprintf(w, "  --image                   抓取同前缀图片 URL（默认关闭）\n")
	fmt.Fprintf(w, "  --video                   抓取同前缀视频 URL （默认关闭）\n")
	fmt.Fprintf(w, "  --vedio                   --video 的兼容别名 （默认关闭）\n")
	fmt.Fprintf(w, "  --txt                     抓取同前缀文本文件 URL（.md .txt .yaml .yml .json 等）（默认关闭）\n")
